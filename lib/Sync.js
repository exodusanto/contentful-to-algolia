...
class Sync {
   ...
   * @return {void}
   */
  syncSingle (type, indexName, params, callback, entryId) {
    new Contentful(this.config.contentful, this.config.locales, params)
      .getEntries(type, entryId)
      .then((content) => {
        // console.log(CircularJSON.stringify(content));
   ...
   * @param  {String}   indexName Algolia index
   * @param  {Array}    params    Contentful custom params
   * @param  {Function} callback  Callback, which is fired when each entry is
   *                              loaded
   * @param  {String}   entryId   Id of an entry that should be syced
   * @return {void}
   */
  sync (contentTypes, indexName, params = {}, callback, entryId = false) {
    ...
    contentTypes.forEach((type) => {
      this.syncSingle(type, indexName, params, callback, entryId);
    });
  }
}
