  ...
  _deepExtract(entry, fields, locale){
    ...
    Object.keys(entry).forEach(e => {
      if(fields.hasOwnProperty(e)){
        cleanEntry[e] = entry[e]; 
      }
    });


    Object.entries(cleanEntry).forEach( d => {
      let entryProp = d[0];
      let entryValue= d[1];
      ...
      if(fields.hasOwnProperty(entryProp)){
        ...
        if(Array.isArray(value)){
          ...
          value = this._extractData(value);
          if(Object.keys(fields[entryProp]).length > 0)
            value = this._deepExtract(value, fields[entryProp], locale);
